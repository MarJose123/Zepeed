...
final class CsvWriterService
{
    /**
     * Write rows to a CSV file on the local private disk and return the
    ...
    public function write(iterable $rows, array $headers, string $filename): string
    {
        ...
        foreach ($rows as $row) {
            // Map positional values back onto the named headers so
            // SimpleExcel always emits a consistent header row.
