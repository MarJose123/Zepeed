...
class ApiException extends Exception
{
    ...
    public function renderApiException(Throwable $exception): JsonResponse
    {
        ...
        $responseData = self::prepareApiExceptionData($exception);
     ...
     * @return array
     */
    private static function prepareApiExceptionData(Throwable $exception): array
    {
        ...
        if ($exception instanceof QueryException) {
            ...
            $responseData['message'] = sprintf('Unable to locate the %s you requested.', self::modelNotFoundMessage($exception->getPrevious()));
            ...
            $responseData['message'] = self::prepareExceptionMessage($exception);
            ...
            if ($debug = self::extractExceptionData($exception)) {
     ...
     * @return string
     */
    private static function prepareExceptionMessage(Throwable $exception): string
    {
     ...
     * @return string
     */
    private static function modelNotFoundMessage(ModelNotFoundException $exception): string
    {
    ...
    private static function extractExceptionData(Throwable $exception): array
    {
        if (config('app.debug') && ! ($exception instanceof HttpExceptionInterface)) {
            $data = [
                ...
                'trace'     => collect($exception->getTrace())->map(static fn ($trace) => Arr::except($trace, ['args']))->all(),
            ];
        } else {
            $data = [];
        }
