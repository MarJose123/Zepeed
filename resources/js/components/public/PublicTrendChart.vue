    ...
    <div class="bg-card border-border rounded-lg border p-3.5">
        <div class="mb-2.5 flex items-center justify-between">
            ...
            <p class="text-sm font-bold">
                {{ currentValue }}<span class="text-muted-foreground ml-1 text-xs font-normal">{{ unit }}</span>
            </p>
        </div>
        ...
        <div v-else class="text-muted-foreground flex h-12 items-center justify-center text-xs">
            No data yet
        </div>
        <p class="text-muted-foreground mt-1.5 text-[10px]">Average across all providers · last 24 h</p>
    </div>
</template>
