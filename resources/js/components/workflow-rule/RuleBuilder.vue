...
import ActionRow from "@/components/workflow-rule/ActionRow.vue";
import ConditionRow from "@/components/workflow-rule/ConditionRow.vue";
...
const form = useForm({
    ...
    conditions: (props.rule?.conditions ?? []) as WorkflowRuleCondition[],
...
const updateAction = (index: number, updated: WorkflowRuleAction) => {
...
const save = () => {
    ...
    const routeName = props.isNew
        ? "speedtest.workflow-rules.store"
        : "speedtest.workflow-rules.update";
    const routeParams = props.isNew ? {} : { workflowRule: props.rule!.id };
        ...
        >
            <div>
                <div class="text-sm font-medium">
                    {{ isNew ? "New workflow rule" : rule?.name }}
                </div>
