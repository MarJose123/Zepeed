...
import type { WorkflowRule } from "@/types/workflow-rule";
...
const breadcrumbs: TBreadcrumbItem[] = [
    { title: "Settings", href: "#" },
    {
        ...
        href: route("speedtest.workflow-rules.index", {}, false),
    },
];
...
watch(
    () => page.flash?.workflow_rule_id as string | null,
    (id) => {
        if (!id) return;
...
const deleteTarget = ref<WorkflowRule | null>(null);
...
const doDelete = () => {
    ...
    router.delete(
        route(
            "speedtest.workflow-rules.destroy",
            { workflowRule: target.id },
            false,
        ),
        {
            preserveScroll: true,
...
const toggle = (rule: WorkflowRule) => {
    router.post(
        route(
            "speedtest.workflow-rules.toggle",
            { workflowRule: rule.id },
            false,
        ),
        {},
        { preserveScroll: true },
...
const lastTriggeredLabel = (rule: WorkflowRule): string => {
...
const actionsSummary = (rule: WorkflowRule): string => {
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full min-h-0 flex-1">
            ...
            >
                ...
                >
                    <div>
                        <h1 class="text-sm font-medium">Workflow rules</h1>
